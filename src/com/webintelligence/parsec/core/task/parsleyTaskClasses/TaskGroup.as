...
public class TaskGroup extends Task {
    ...
    public function TaskGroup () {
        ...
        if (logger == null) {
            logger = LogContext.getLogger(TaskGroup);
        }
        allTasks = new List();
        activeTasks = new List();
    ...
    public override function get cancelable () : Boolean {
        ...
        for (var i : int = 0; i < allTasks.size(); i++) {
            var t : Task = Task(allTasks.getAt(i));
    ...
    public function removeAllTasks () : void {
        for (var i:uint = 0; i < allTasks.size(); i++) {
            var t:Task = Task(allTasks.getAt(i));
            t.setParent(null);
        }
        allTasks.removeAll();
    ...
    public function getTask (index : uint) : Task {
        return Task(allTasks.getAt(index));
    }
    ...
    protected override function doSuspend () : void {
        locked = true;
        try {
            for (var i:Number = 0; i < activeTasks.size(); i++) {
                var t:Task = Task(activeTasks.getAt(i));
